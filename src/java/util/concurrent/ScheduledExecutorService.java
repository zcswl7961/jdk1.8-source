...
/**
 *
 * 在ExecutorService得基础上提供了在一定延迟后或者定时进行任务调度的的功能
...
public interface ScheduledExecutorService extends ExecutorService {
    ...
    /**
     * 一次性延迟操作，创建并执行一次操作，该操作在给定得延迟后执行
     ...
     * 如果任务的运行时间超过了设置的period间隔，那么后续任务会晚一点运行，
     * 不会有两个任务并发运行的情况
     * @param command the task to execute
    ...
    /**
     * scheduleAtFixedRate相对于开始时间，任务启动频率固定。
     *
     * 而该方法是相对于上一次任务执行时间，启动频率固定。即运行时间为:
     *
     * 第一次：now + initialDelay
     * 第二次: 第一次任务完成时间 + delay
     * 第三次: 第二次任务完成时间 + delay
     *
     * 针对上一个任务执行完毕之后再次执行
