...
public abstract class AbstractQueuedSynchronizer
     ...
     * and interrupts.
     *
     * 前驱节点（not used in original CLH locks 这个意思貌似是head节点不使用）
     * 需要处理取消事宜（cancellation） 如果一个节点被取消（实际上是节点有一个获取锁的机会）
     * 他的子节点需要重新链接到一个不取消（node waitStatus 不为1）的前驱节点，
     ...
     *
     * 我们同样使用了next 进行实现阻塞机制
     * 每一个node节点都保存了对应的线程信息  node  -> thread
     * 前驱节点通过其next来决定唤醒哪一个线程，并确保下一个新生成的queue（可能又来了一个node加入到锁等待队列中）不去和它的前辈去竞争这个锁
    ...
    private final boolean parkAndCheckInterrupt() {
        ...
        // 获取当前线程的中断标识,这个应该由谁来出现处理这个线程的thread.interceptor 呢？
    ...
    private void doAcquireSharedInterruptibly(int arg)
        throws InterruptedException {
        // 进入到这里，首先是要把当前线程封装成Node加入得AQS得CLH得等待队列中
        // 这个和ReentrantLock一样，内部自旋，并将封装得Node追加到CLU 得tail，当然初始head为null得话，实例一个虚拟节点得head
        final Node node = addWaiter(Node.SHARED);
        boolean failed = true;
        try {
            for (;;) {
                // 自旋，首先是查看当前节点在CLH等待队列得前驱节点
                final Node p = node.predecessor();
                if (p == head) {
                    // 这个太熟悉了，前驱节点为head，说明都没人在等待信号量了，那我当然要试一下了
                    // 内部又是一阵自旋，跳出来有两个可能，
                    //      1 我拿到了，哈哈，返回得r 肯定是 r >= 0 ，
                    //      2，我没拿到，许可没得了  （注意这里内部不可能会出现 if (hasQueuedPredecessors()) return -1;）
                    int r = tryAcquireShared(arg);
                    if (r >= 0) {
                        // 说明拿到锁了
                        setHeadAndPropagate(node, r);
                ...
                // 这个是不是贼熟悉，跳到这说明CLH等待队列至少你得前驱节点是一只苦苦等着许可得，
                // 公平原则你得靠边
                // shouldParkAfterFailedAcquire和ReentrantLock套路一样， 修改节点得waitStatus
                // 当前节点挂起之前，肯定是它得前驱节点修改成了 SIGNAL
                if (shouldParkAfterFailedAcquire(p, node) &&
                        // 这个就和ReentrantLock一样了，调用 LockSupport.park(this); 挂起了线程
                    parkAndCheckInterrupt())
                    throw new InterruptedException();
            }
        } finally {
            if (failed)
                // 终于啊，我拿到了，我要t把我的waitSatus 改成CANCELLED
                cancelAcquire(node);
        }
    }
    ...
    public final void acquire(int arg) {
        if (!tryAcquire(arg) &&
                // 如果成功返回true，那就更好呀，这种情况下有两种，1，线程获取可重入的锁，2，当前的锁没有任何线程所持有，即AQS中的status == 0，于此同时通过cas成功修改了status = 1
                // 假如一个线程获取锁失败了，进入到这个方法
